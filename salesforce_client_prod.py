...
class SalesforceClientCC:
    ...
    def __init__(self, cfg: SalesforceCCConfig, timeout_s: float = 30.0) -> None:
        ...
        self.RecordTypes = RecordTypes(
            Account=AccountRecordTypes(
                Business= _require_record_type_env("PROD_SF_ACCOUNT_RT_BUSINESS"),
                Person= _require_record_type_env("PROD_SF_ACCOUNT_RT_PERSONAL"),
    )
        )
...
def load_salesforce_cc_config_from_env() -> SalesforceCCConfig:
    required = ["PROD_SF_TOKEN_URL", "PROD_SF_CLIENT_ID", "PROD_SF_CLIENT_SECRET"]
    missing = [k for k in required if not os.getenv(k)]
    if missing:
        raise RuntimeError(f"Missing env vars: {', '.join(missing)}")

    return SalesforceCCConfig(
        token_url=os.getenv("PROD_SF_TOKEN_URL", ""),
        client_id=os.getenv("PROD_SF_CLIENT_ID", ""),
        client_secret=os.getenv("PROD_SF_CLIENT_SECRET", ""),
        api_version=os.getenv("PROD_SF_API_VERSION", "60.0"),
    )
