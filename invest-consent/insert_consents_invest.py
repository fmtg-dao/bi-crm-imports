...
# --- Logging Setup ---
log_path = REPO_ROOT / "logs/insert_consents_invest.log"
...
def save_skipped_rows(rows: list[dict], found: dict[str, list[dict]], batch_id: str, run_stamp: str) -> None:
    out_path = REPO_ROOT / f"local_data/skipped_invest_consents_{batch_id}_{run_stamp}.json"
...
def save_failed_records(failed: list[dict], batch_index: int, batch_id: str) -> None:
    out_path = REPO_ROOT / f"local_data/failed_invest_consents_{batch_id}_batch_{batch_index}.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    if len(args) != 1:
        raise SystemExit("Aufruf: insert_consents_invest.py <batch_id> [--dry-run]")
    batch_id = args[0]
    ...
    print(f"insert_consents_invest | start: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    if dry_run:
        ...
        out_path = REPO_ROOT / f"local_data/dry_run_invest_consents_{batch_id}.csv"
        out_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    print(f"\ninsert_consents_invest | end: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  -> Erfolgreich:           {len(succeeded_row_ids)}")
    print(f"  -> Fehlgeschlagen:        {len(total_failed)}")

    if total_failed:
        all_failed_path = REPO_ROOT / f"local_data/batch/all_failed_invest_consents_{batch_id}.json"
