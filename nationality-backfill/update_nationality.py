...
# ein Direktaufruf aus nationality-backfill/ muessen in denselben Ordnern landen.
REPO_ROOT = Path(__file__).resolve().parents[1]

# --- Logging Setup ---
log_path = REPO_ROOT / "logs/update_nationality.log"
...
# --- Constants ---
OBJECT_NAME = "Account"
# 2.669 Records insgesamt: ein einziger Bulk-Job reicht, die 20.000er
# Optimierung aus dem Billing-Backfill braucht es hier nicht.
BATCH_SIZE = 5000
...
# Staging-Spalte -> Salesforce-Feld. Quelle sind die stg_imp_investors_*-
# Tabellen (ISO-2, Picklist-Werte alle bereits live in SF vorhanden, siehe
# 02_recon_nationality.sql Query 4).
FIELD_MAP = {
    "nationality_country_code": "NationalityCountryCode__pc",
}
...
def group_by_signature(records: list[dict]) -> dict[tuple, list[dict]]:
    """
    Gruppiert Records nach der Menge ihrer belegten Felder. Bei einem einzigen
    Feld kollabiert das zu genau einer Gruppe — der Schutz gegen leere
    CSV-Zellen bleibt trotzdem drin, er kostet nichts.
    """
...
def accounts_with_nationality(sf: SalesforceClientCC, account_ids: list[str]) -> set[str]:
    """
    Live-Check vor dem Schreiben: Accounts, bei denen NationalityCountryCode__pc
    inzwischen belegt ist, werden uebersprungen (Entscheidung: nur schreiben,
    wo das Ziel leer ist). Der Spiegel kann Stunden alt sein, die Wahrheit
    liegt in Salesforce.
    """
    ...
    for chunk in chunked(account_ids, 400):
        ...
        soql = (
            ...
            f"WHERE Id IN ({ids}) AND NationalityCountryCode__pc != null"
        )
...
def save_skipped_rows(rows: list[dict], batch_id: str, run_stamp: str) -> None:
    out_path = REPO_ROOT / f"local_data/skipped_nationality_backfill_{batch_id}_{run_stamp}.json"
    payload = [
        {
            "row_id": r.get("row_id"),
            "sf_account_id": r.get("sf_account_id"),
            "email": r.get("email"),
            "reason": "NationalityCountryCode__pc inzwischen belegt (live oder frueherer Lauf)",
        }
        for r in rows
    ]
...
def save_failed_records(failed: list[dict], job_index: int, batch_id: str) -> None:
    out_path = REPO_ROOT / f"local_data/failed_nationality_backfill_{batch_id}_job_{job_index}.json"
    write_json(out_path, failed)
    print(f"  -> {len(failed)} fehlgeschlagene Records gespeichert: {out_path}")


# --- DB Helpers ---
def mark_nationality_processed(db: MySQLClient, batch_id: str, row_ids: list[int]) -> None:
    if not row_ids:
        return
    ...
    with db.transaction() as conn:
        for chunk in chunked(row_ids, 1000):
            ...
            sql = f"""
                ...
                SET    _nationality_processed_at = NOW()
                WHERE  _batch_id = %s
        ...
        for chunk in chunked(row_ids, 1000):
            ...
            sql = f"""
                ...
                WHERE  _batch_id = %s
                  AND  row_id    IN ({placeholders})
            """
            db.execute(sql, [f"nationality_present_{run_stamp}", batch_id, *chunk], conn=conn)
...
def main():
    ...
    if len(args) != 1:
        raise SystemExit("Aufruf: update_nationality.py <batch_id> [--dry-run]")
    batch_id = args[0]

    print(f"update_nationality | start: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    if dry_run:
        ...
        for gi, (sig, recs) in enumerate(sorted(groups.items(), key=lambda kv: -len(kv[1])), 1):
            out_path = out_dir / f"dry_run_nationality_backfill_{batch_id}_sig{gi}.csv"
    ...
    with SalesforceClientCC(cfg_sfcc) as sf:
        ...
        # 4. Live-Skip-Check: Accounts, deren NationalityCountryCode__pc seit
        #    dem Spiegel-Stand belegt wurde, nicht anfassen. Uebersprungene
        #    Zeilen behalten _nationality_processed_at = NULL und tauchen im
        #    skipped_*-File auf.
        run_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        print("  -> Live-Check: NationalityCountryCode__pc bereits belegt?")
        taken = accounts_with_nationality(sf, [str(r["sf_account_id"]) for r in rows])
        if taken:
        ...
        for sig, group_records in sorted(groups.items(), key=lambda kv: -len(kv[1])):
            for batch_records in chunked(group_records, BATCH_SIZE):
                ...
                try:
                    # Vor jedem Job frisch anmelden: kostet nichts und macht den
                    # Lauf unempfindlich gegen abgelaufene Client-Credentials-Tokens.
                    sf.authenticate()
                    ...
                    if job_row_ids:
                        mark_nationality_processed(db, batch_id, job_row_ids)
    ...
    print(f"\nupdate_nationality | end: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  -> Erfolgreich:    {n_succeeded:,}")
    print(f"  -> Fehlgeschlagen: {len(total_failed):,}")

    if total_failed:
        all_failed_path = REPO_ROOT / f"local_data/batch/all_failed_nationality_backfill_{batch_id}.json"
