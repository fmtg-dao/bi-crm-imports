...
# --- Logging Setup ---
log_path = Path("logs/update_loyalty_members_bulk.log")
...
def row_to_sf_record(row: dict) -> dict:
    """
    Mappt eine Zeile aus crm_imp_person_accounts auf ein Salesforce LoyaltyProgramMember-Update.
    Update erfolgt per Salesforce Id (sf_loyalty_member_id) → muss als 'Id' Spalte im CSV stehen.

    None-Felder werden am Ende entfernt, damit Bulk API sie NICHT überschreibt.
    """
    record = {
        # --- Pflicht: SF Id für Update-Operation ---
        "Id":                                   row.get("sf_loyalty_member_id"),

        # --- Loyalty Membership Stammdaten ---
        #"MembershipNumber":                     row.get("loyalty_membership_number"),
        #"EnrollmentDate":                       sf_date(row.get("loyalty_enrollment_date")),
        ...
        #"SourceOrigin__c":                      row.get("source_origin"),
    }
...
def save_failed_records(failed: list[dict], batch_index: int, batch_id: str) -> None:
    out_path = Path(f"local_data/failed_loyalty_members_{batch_id}_batch_{batch_index}.json")
...
def mark_batch_processed(db: MySQLClient, batch_id: str, sf_ids: list[str]) -> None:
    """Setzt _processed_at = NOW() für erfolgreich verarbeitete Loyalty-Records."""
    if not sf_ids:
        return
    ...
    sql = f"""
        ...
        WHERE  _batch_id            = %s
          AND  sf_loyalty_member_id IN ({placeholders})
    """
...
def main():
    ...
    print(f"update_loyalty_members_bulk | start: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    print(f"  → {len(rows)} Loyalty Members zum Update geladen")
    ...
    if missing_id:
        raise RuntimeError(
            f"{len(missing_id)} Records ohne sf_loyalty_member_id — Update nicht möglich. Abbruch."
        )
        ...
        for i, batch in enumerate(tqdm(batches, desc="Bulk Batches")):
            ...
            try:
                ...
                if status.get("numberRecordsFailed", 0) > 0:
                    ...
                    failed_ids_in_chunk = {
                        f.get("sf__Id") or f.get("Id")
                        for f in failed
                        if f.get("sf__Id") or f.get("Id")
                    }
                ...
                succeeded_in_chunk = [
                    sf_id for sf_id in batch_ids_in_chunk if sf_id not in failed_ids_in_chunk
                ]
    ...
    print(f"\nupdate_loyalty_members_bulk | end: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  → Erfolgreich:           {len(total_succeeded_ids)}")
    print(f"  → Fehlgeschlagen:        {len(total_failed)}")

    if total_failed:
        all_failed_path = Path(f"local_data/batch/all_failed_loyalty_members_{batch_id}.json")
